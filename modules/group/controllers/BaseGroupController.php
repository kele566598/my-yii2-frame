...
class BaseGroupController extends BaseController
{
    ...
    public function beforeAction( $action ){
        ...
        if(!$is_login) {
            if ( \Yii::$app->request->isAjax) {
                ...
                $this->redirect( UrlService::buildGroupUrl("/default/login") );
            }
            return false;
        }

        ApplogService::addMemberLog( $this->current_user['id'] );
        return true;
    }
    ...
    protected function checkLoginStatus(){
        ...
        $auth_cookie = $this->getCookie( Yii::$app->params['group_auth_token'] );
        ...
        if( $id && preg_match("/^\d+$/",$id) ){
            $user_info = Member::findOne([ 'id' => $id,'status' => 1 ]);
            if( !$user_info ){
    ...
    public function setLoginStatus( $user_info ){
        ...
        $this->setCookie(Yii::$app->params['group_auth_token'],$auth_token."#".$user_info['id']);
    }

    protected  function removeAuthToken(){
        $this->removeCookie( Yii::$app->params['group_auth_token'] );
    }
